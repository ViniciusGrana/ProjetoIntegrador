...
import json
from dash import email_remetente
...
def msg_sensor(client, userdata, msg):
    ...
    with app.app_context():
        try:
            ...
            try:
                tempo_oficial = datetime.fromtimestamp(
                    int(tempo_registro), tz=timezone.utc
                )
            ...
            response = model.generate_content(
                "Faça um aviso sobre a temperetura atual para que a população possa se proteger de altas temperaturas. Resuma isto em 3 linhas"
            )
    ...
    if registro:
        try:
            ...
            return generate_response(
                200, "registro", registro.to_json(), "Registro deletado com sucesso"
            )
        except Exception as e:
            print("Erro", e)
            mybd.session.rollback()
...
def create_data():
    try:
        ...
        return generate_response(
            201, "data", novo_registro, "Dados recebidos com sucesso"
        )
...
def to_json_sensor(self):
    return {
        ...
        "tempo_registro": (
            self.tempo_registro.strftime("%Y-%m-%d %H:%M:%S")
            if self.tempo_registro
            else None
        ),
    }
